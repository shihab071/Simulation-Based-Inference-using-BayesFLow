"""Continue the trained LI model from epoch 60 to epoch 80.

The epoch-60 .keras file contains the trained networks, adapter, and
standardization state. This script loads that complete model, starts a fresh
optimizer schedule with a smaller learning rate, and trains for 20 more
epochs. The epoch-40 and epoch-60 models are never overwritten.
"""

import os

# Select the JAX backend before importing Keras or BayesFlow.
...
# Use the same mixture of dataset sizes as the previous training runs.
N_TRIALS_OPTIONS = [500, 1_000, 2_500, 5_000, 10_000]
N_TRIALS_WEIGHTS = [0.30, 0.25, 0.20, 0.15, 0.10]

# Continue from epoch 60 for one final 20-epoch round.
STARTING_EPOCHS = 60
...
# Repeat the same validation design used at epochs 40 and 60.
...
# Load the continued model from the previous round.
SOURCE_MODEL_PATH = CHECKPOINT_DIR / "li_model_approximator_epoch60.keras"

# Save everything from this run under new epoch-80 filenames.
...
def prior():
    """Sample one set of parameters from the training priors."""
    return {
...
def validation_meta():
    """Use 10,000 trials for every held-out validation dataset."""
...
def likelihood(v_A, v_B, beta, b, t0, n_trials):
    ...
    # Code A as 0 and B as 1. Missing DRTs become zero; the separate
    # double-response flag tells the network whether the DRT is meaningful.
    return {
...
def save_figure(figure, filename):
    """Save a diagnostic figure and close it to release memory."""
...
def main():
    ...
    # Stop before doing any work if the epoch-60 model is missing.
    if not SOURCE_MODEL_PATH.is_file():
        raise FileNotFoundError(
            f"Source model was not found: {SOURCE_MODEL_PATH}"
        )

    # Refuse to overwrite an existing epoch-80 result.
    for output_path in [
        CONTINUED_CHECKPOINT_PATH,
        CONTINUED_FINAL_MODEL_PATH,
    ]:
    ...
    # Ignore the optimizer stored in the .keras file. The new workflow will
    # create a fresh optimizer schedule for these 20 epochs.
    ...
    # Pull the adapter and networks directly from the saved model so the
    # continuation cannot drift from the original model specification.
    ...
    # Replace the temporary model created by BasicWorkflow with the complete
    # trained epoch-60 model, including its learned weights.
    ...
    print(f"Final epoch-80 model: {CONTINUED_FINAL_MODEL_PATH}")
    print(f"Epoch-60 model will remain at: {SOURCE_MODEL_PATH}")
    ...
    print(f"Saved epoch-80 model to {CONTINUED_FINAL_MODEL_PATH}")

    # Compare the final four epochs with the preceding four. This is only a
    # trend check, not a formal convergence test.
    ...
    if loss_drop > 0.05:
        print("Loss is still declining; interpret this with the diagnostics.")
    else:
    ...
    print("\n" + "=" * 72)
    print("EPOCH-80 CONTINUATION COMPLETED SUCCESSFULLY")
    print("=" * 72)
    print(f"Epoch-60 model preserved: {SOURCE_MODEL_PATH}")
    print(f"New epoch-80 model: {CONTINUED_FINAL_MODEL_PATH}")
    print("Review all epoch-80 diagnostics before fitting the real data.")
